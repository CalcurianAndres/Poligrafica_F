    ...
    async function generarPDF(){
      ...
      pdf.add(
        new Table([
          ...
          [
            new Cell( new Txt(`PRODUCTO: ${PRODUCTO.producto.producto}`).end).colSpan(2).end,
            ''
          ]
        ]).widths(['60%','40%']).end
      )
      ...
      pdf.add(
        new Table([
          [
            'PELICULA','IMPRESIÓN','POST-IMPRESION'
          ],
          [
            new Cell( new Txt(`${PRODUCTO.cliente.codigo} - ${PRODUCTO.producto.codigo}`).end).end,
